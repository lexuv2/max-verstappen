...
# Args: [num_episodes] [model_name] [plot_name]
...
def plot_duration(show_result: bool = False) -> None:
    ...
    if show_result:
        plt.title(f'Result ({num_episodes} episodes)')
    else:
        plt.clf()
        plt.title(f'Training... ({len(durations_t)}/{num_episodes})')
    plt.xlabel('Episode')
    plt.ylabel('Duration')
...
if __name__ == '__main__':
    ...
    plt.ioff()
    if len(sys.argv) >= 4:
        plt.savefig(f"{sys.argv[3]}.png", bbox_inches='tight')
    plt.show()
    ...
    if len(sys.argv) >= 3:
        ...
        torch.save(dict,f"{sys.argv[2]}.pt")
